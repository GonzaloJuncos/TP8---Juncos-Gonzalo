...
def calcular_metricas(df):
    ...
    # Validar columnas numéricas para evitar divisiones por cero o NaN
    df = df[(df['Unidades_vendidas'] > 0) & (df['Ingreso_total'] > 0)]
    
    # Calcular precio promedio y margen
...
def procesar_datos(df, sucursal_seleccionada):
    ...
    for producto in df['Producto'].unique():
        ...
        unidades_totales_producto = datos_producto['Unidades_vendidas'].sum()
                # Calcular el precio promedio como una relación global entre ingresos y unidades
                # Calcular el precio promedio y truncar a dos decimales
                # Calcular el precio promedio redondeado al entero más cercano
        precio_promedio = round(np.mean(datos_producto['Ingreso_total'] / datos_producto['Unidades_vendidas']))



                # Calcular y redondear el margen promedio al entero más cercano
